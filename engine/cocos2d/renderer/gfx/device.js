...
function _commitVertexBuffers(device, gl, cur, next) {
  const instancingExt = device._instancingExt;
  const divisorFunc = device._vertexAttribDivisor;
  ...
  if (cur.maxStream !== next.maxStream) {
    ...
    for (let i = 0; i < next.maxStream + 1; ++i) {
      if (
        ...
        cur.vertexBufferDivisors[i] !== next.vertexBufferDivisors[i]
      ) {
        attrsDirty = true;
        break;
      }
  ...
  if (attrsDirty) {
    ...
    for (let i = 0; i < next.maxStream + 1; ++i) {
      ...
      let vbDivisor = next.vertexBufferDivisors[i] || 0;
      if (!vb || vb._glID === -1) {
        continue;
      }
      ...
      for (let j = 0; j < next.program._attributes.length; ++j) {
        ...
        if (divisorFunc) {
          divisorFunc.call(instancingExt || gl, attr.location, vbDivisor || el.divisor || 0);
        }
      }
    }
    ...
    for (let i = 0; i < device._caps.maxVertexAttribs; ++i) {
      if (device._enabledAttributes[i] !== device._newAttributes[i]) {
        if (divisorFunc) {
          divisorFunc.call(instancingExt || gl, i, 0);
        }
...
export default class Device {
  ...
  setVertexBuffer(stream, buffer, start = 0, divisor = 0) {
    ...
    this._next.vertexBufferDivisors[stream] = divisor;
  ...
  setInstanceCount(count) {
    this._next.instanceCount = count > 0 ? count : 0;
  }
  ...
  draw(base, count, instances = 0) {
    ...
    if (count) {
      const instanceCount = instances > 0 ? instances : next.instanceCount;

      // drawPrimitives
      if (instanceCount > 0 && this._caps.supportInstancing) {
        if (next.indexBuffer && this._drawElementsInstanced) {
          this._drawElementsInstanced.call(
            this._instancingExt || gl,
        ...
        } else if (!next.indexBuffer && this._drawArraysInstanced) {
          this._drawArraysInstanced.call(
            this._instancingExt || gl,
            this._next.primitiveType,
            base,
            count,
