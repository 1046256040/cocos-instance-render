...
export default class ForwardRenderer extends BaseRenderer {
  constructor(device, builtin) {
    ...
    // 使用 JavaScript Number（双精度64位）累加时间，避免 Float32 累加误差
    // 只在设置 uniform 时转换为 Float32，减少精度损失
    this._timeAccumulator = 0.0;  // 使用双精度累加器
  ...
  render(scene, dt) {
    ...
    if (dt) {
      ...
      // 将时间拆分成整数部分和小数部分，分别存储以提升精度
      // cc_time.z = 整数秒部分（为保证 Float32 精度，这里将时间取模到一个有限区间内）
      // cc_time.w = 小数秒部分（0-1之间，Float32 在这个范围内精度最高）
      // 在 shader 中使用 cc_time.z + cc_time.w 组合得到高精度时间值
      const timeInCycle = this._timeAccumulator % 20;
      const integerPart = Math.floor(timeInCycle);
      const fractionPart = timeInCycle - integerPart;  // 小数部分（0-1之间）

      // 保持向后兼容：cc_time.x 和 cc_time.y 保持不变
      this._time[0] = this._timeAccumulator;  // 完整时间值（向后兼容）
      this._time[1] = dt;                      // 帧时间差（向后兼容）
      this._time[2] = integerPart;            // 整数秒部分（高精度时间）
      this._time[3] = fractionPart;           // 小数秒部分（高精度时间）
    }
