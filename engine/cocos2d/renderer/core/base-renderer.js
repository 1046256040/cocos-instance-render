...
export default class Base {
  ...
  _draw (item) {
    ...
    for (let i = 0; i < passes.length; ++i) {
      ...
      let count = ia.count;
      let hasInstancedStream = false;
      if (ia._vertexBufferDivisors) {
        for (let stream = 0; stream < ia._vertexBufferDivisors.length; ++stream) {
          if ((ia._vertexBufferDivisors[stream] || 0) > 0) {
            hasInstancedStream = true;
            break;
          }
        }
      }

      if (hasInstancedStream && (!ia._instanceCount || ia._instanceCount <= 0)) {
        continue;
      }

      if (ia._vertexBuffers && ia._vertexBuffers.length > 0) {
        for (let stream = 0; stream < ia._vertexBuffers.length; ++stream) {
          let vertexBuffer = ia._vertexBuffers[stream];
          if (!vertexBuffer) {
            continue;
          }
          ...
          device.setVertexBuffer(
            stream,
            vertexBuffer,
            ia._vertexBufferOffsets[stream] || 0,
            ia._vertexBufferDivisors[stream] || 0
          );
        }
      }
      else if (ia._vertexBuffer) {
      ...
      // draw pass
      device.setInstanceCount(ia._instanceCount || 0);
